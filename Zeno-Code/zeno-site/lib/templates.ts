...
每份报价的范围、金额和项目阶段都不同，我不会用一个未经核对的平均金额替您下结论。
...
如果您现在手里有报价单，可以先做免费初筛工具扫一眼。做完如果您觉得"这东西我自己也能看"——那可能真的不用找我。如果做完您心里更没底了——那种"不确定感"就是我的价值。`,
  },
  {
    ...
    content: `审查意见本身不能强迫施工方接受，也不能保证项目不发生增项或争议。

我能做的是把模糊条款定位到原文，整理需要追问、修改和留痕的内容。最终是否修改，要由您和合同相对方确认；施工中的争议仍要回到合同、变更记录和现场证据。

开始前会把本次材料范围、交付物和后续澄清边界写清楚，不承诺替代法律、造价或现场监理。`,
  },
...
控制预算不能只等到施工阶段。签约前要先把报价单里模糊的范围、计量、材料和变更规则问清楚。${SERVICE_PRICING.renovationSpecialist.displayPrice} 装修专项判断会围绕一份约定材料或一个决策节点整理这些问题，但不能承诺后续一定不增加费用。
...
${SERVICE_PRICING.renovationSpecialist.displayPrice} 装修专项判断可以把合同和报价放在一起核对，整理需要继续确认和修改的条款；是否修改及最终法律效力，仍由双方和必要的专业人员确认。`,
  },
  {
...
简单说下我做什么：帮业主在签约前核对报价单和合同，把需要继续确认的范围、计量、材料、付款和责任边界整理出来。
...
免费初筛工具不花钱，可以先定位哪些边界还没写清。它不能证明报价一定没问题；如果做完仍然有一个具体问题，可以先看 ${SERVICE_PRICING.diagnosis.displayPrice} 单问题判断诊断，完整报价和合同再进入 ${SERVICE_PRICING.renovationSpecialist.displayPrice} 装修专项判断。
