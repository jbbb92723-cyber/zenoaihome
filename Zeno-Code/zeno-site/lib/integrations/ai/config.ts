export const AI_TASKS = ['public_chat', 'public_vision', 'admin_draft', 'classification'] as const

export type AiTask = (typeof AI_TASKS)[number]
export type AiProviderId = 'deepseek' | 'zhipu' | 'openai' | 'custom'
...
const TASK_ENVIRONMENTS: Record<AiTask, { provider: string; model: string }> = {
  ...
  public_vision: { provider: 'AI_PUBLIC_VISION_PROVIDER', model: 'AI_PUBLIC_VISION_MODEL' },
...
function normalizeProvider(value: string | undefined): AiProviderId | null {
  ...
  if (provider === 'deepseek' || provider === 'zhipu' || provider === 'openai' || provider === 'custom') {
    return provider
  }
...
function inferProvider(task: AiTask): AiProviderId | null {
  ...
  const explicit = normalizeProvider(
    process.env[taskEnvironment.provider]
      ?? (task === 'public_vision' ? undefined : process.env.AI_DEFAULT_PROVIDER),
  )
  if (explicit) return explicit

  if (task === 'public_vision') {
    return process.env.AI_ZHIPU_API_KEY ? 'zhipu' : null
  }

  if (process.env.AI_DEEPSEEK_API_KEY || process.env.DEEPSEEK_API_KEY) return 'deepseek'
  if (process.env.AI_ZHIPU_API_KEY) return 'zhipu'
...
function providerValues(provider: AiProviderId, task: AiTask) {
  ...
  if (provider === 'zhipu') {
    return {
      ...
      model: task === 'public_vision'
        ? (process.env.AI_ZHIPU_VISION_MODEL ?? 'glm-5v-turbo')
        : (process.env.AI_ZHIPU_CHAT_MODEL ?? 'glm-5.3'),
    }
  }
...
export function resolveAiTaskConfig(task: AiTask): AiTaskConfig | null {
  ...
  const model = process.env[taskEnvironment.model]
    ?? (task === 'public_vision' ? values.model : process.env.AI_DEFAULT_MODEL ?? values.model)
