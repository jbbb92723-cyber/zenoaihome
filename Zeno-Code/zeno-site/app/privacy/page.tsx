...
const sections = [
  {
    ...
    body: [
      ...
      'Zeno 助手会把最近最多 24 条文字对话保存在你当前浏览器的本地存储中，7 天后自动失效，也可以通过对话框里的“重新开始”立即清除。这些本地记录不会因此成为账号聊天档案；每次继续对话时，最近最多 5 轮文字会随当前问题发送给 AI 服务商。',
      '你主动添加的单张图片只随当前一轮请求发送给当前配置的 AI 服务商。本站不会把图片内容或文件名写入聊天本地记录或网站数据库；文字历史只会保留“该轮曾有临时图片”的提示，后续轮次不会再次发送原图。',
    ],
  },
  {
    title: '这些信息用来做什么',
  ...
  },
  {
    ...
    body: [
      ...
      '提交文字或图片给 AI 服务商前，请主动遮挡或删除身份证号、银行卡信息、密码、门牌地址、未公开的客户隐私和其他不必要的敏感信息。图片识别可能不完整，金额、数量、日期和责任条款必须对照原件确认。',
    ],
  },
]
