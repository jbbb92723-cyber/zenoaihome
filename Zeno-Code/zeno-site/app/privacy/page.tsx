...
const sections = [
  {
    ...
    body: [
      ...
      '当你使用“我的装修档案”时，我们会保存你主动上传的报价原件、文件名称和大小、解析后的预算条目、你对条目的修改与确认、报价版本及版本差异。原件保存在私有对象存储中，不使用永久公开链接。',
  ...
  },
  {
    ...
    body: [
      ...
      '用于在你的账号内生成预算结构、保留报价版本、比较变化和导出记录。自动提取结果只用于帮助整理材料，不表示赞诺已经审核，也不构成造价、合同或法律结论。',
      '用于排查异常请求、限制滥用、维护网站安全，以及在获得必要依据后改进内容、工具和交付流程。',
      '如果你使用站内 AI 功能，相关输入可能发送给当前配置的 AI 服务商用于生成结果。装修档案首版的 CSV 和 XLSX 采用确定性解析；未来某项文件需要 OCR 或 AI 识别时，会在处理前另行说明。AI 输出仅作辅助，不替代合同、法律、财务、医疗或其他专业判断。',
    ],
  },
  {
    title: '谁可以访问',
    body: [
      ...
      '装修档案默认供对应账号使用。只有你主动申请人工审核并确认材料范围后，赞诺才会把你指定的报价版本作为人工审核材料；后续上传的新版本不会自动改变已经确认的审核范围。',
      ...
      '装修档案中的原件、解析结果和修改记录不会自动进入公开文章、风险词典、案例库或 AI 公共知识库。需要将其中内容用于公开复盘时，会另行确认授权和去标识化范围。',
    ],
  },
  {
    title: '保存多久',
    body: [
      ...
      '你可以在装修档案中申请删除整份档案。档案会立即从账号中隐藏，系统随即尝试清理私有存储中的对应原件；上传凭证失效并经过安全清理窗口后，才最终删除在线数据库中的预算版本和条目。失败时会保留删除标记并重试；备份或基础设施缓存中的副本可能按其正常周期延后清理。',
  ...
  },
  {
    ...
    body: [
      ...
      '上传前请确认你有权处理这些材料。建议先删除身份证号、银行卡号、门锁密码、精确住址、无关人员联系方式以及其他完成预算整理并不需要的信息。',
