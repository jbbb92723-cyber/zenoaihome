...
import os, sys, json, re, time, base64, subprocess, shutil, argparse, textwrap, tempfile
...
def engine_chrome_split(html_path, pdf_path, meta=None, brand_spec=None):
    ...
    if b64_match:
        ...
        tmp_logo = os.path.join(tempfile.gettempdir(), f"opc_pdf_logo_{os.getpid()}.png")
    ...
    def _run_chrome(html_str, out_pdf, wait=130):
        ...
        tmp_html = os.path.join(tempfile.gettempdir(), f"opc_chunk_{out_pdf.stem}_{os.getpid()}.html")
        cdir     = os.path.join(tempfile.gettempdir(), f"opc_cdir_{out_pdf.stem}_{os.getpid()}")
    ...
    tmp_dir     = Path(tempfile.gettempdir()) / f"opc_split_{os.getpid()}"
...
def engine_chrome_pagedjs(html_path, pdf_path):
    ...
    try:
        ...
        temp_dir = os.path.join(tempfile.gettempdir(), f"chrome-pdf-{os.getpid()}")
