...
const SYSTEM_PROMPT_ZH = `你是 Zeno（赞诺）的网站 AI 助手。Zeno 是一位有 16 年装修一线经验、现在专注做签约前报价风险判断的人。

【你的使命】
帮助准备签装修合同的业主先看清报价、合同和付款节点里没说清的风险。
...
- 服务价格（¥99 报价风险初查 / ¥299 标准报价快审 / ¥699 深度版签约前判断）：/services/renovation
- 联系 Zeno：/contact
- 文章博客：/blog
...
const ROUTE_LABELS: Record<'zh' | 'en', Record<string, string>> = {
  zh: {
    ...
    '/services/renovation': '看服务价格',
    '/services/renovation#quote-entry': '了解报价风险初查',
    '/services/renovation#quote-standard': '了解标准报价快审',
    '/services/renovation#quote-deep': '了解深度版签约前判断',
...
const FOLLOW_UPS_ZH: Record<IntentKey, string[]> = {
  ...
  service: ['¥99 和 ¥299 怎么选', '什么时候需要 ¥699 深度判断', '我先用工具还是先咨询'],
  ai: ['AI 能帮我整理报价吗', 'AI 不能替我判断什么', '我该先做报价初筛吗'],
  tools: ['我现在最该先用哪个入口', '检查模板先看哪一份', '风险词典先查哪些词'],
...
const KNOWLEDGE_ZH: Array<{ pattern: RegExp; intent: IntentKey; payload: ChatReplyPayload }> = [
  {
    ...
    payload: {
      ...
      actions: [
        ...
        { label: '看人工服务', href: '/services/renovation', kind: 'service' },
      ],
  ...
  },
  {
    ...
    intent: 'service',
    payload: {
      reply: '现在主线只保留三项人工判断：¥99 报价风险初查、¥299 标准报价快审、¥699 深度版签约前判断。不是所有情况都需要直接买服务，能先用免费初筛和检查模板看清的，先看清再说。',
      bullets: [
        '只有几行重点报价，优先看 ¥99 报价风险初查。',
        '已有完整报价、准备继续谈或签约，优先看 ¥299 标准报价快审。',
        '报价、合同和付款节点都已经有，且临近签约，再看 ¥699 深度版签约前判断。',
      ],
      actions: [
        { label: '看服务说明', href: '/services/renovation', kind: 'service' },
        { label: '直接联系 Zeno', href: '/contact', kind: 'contact' },
        { label: '先用工具判断', href: '/tools/quote-check', kind: 'tool' },
      ],
  ...
  },
  {
    ...
    payload: {
      ...
      bullets: [
        '先把报价单里的项目、单位、数量、单价和备注整理清楚。',
        '再让 AI 辅助标记漏项、模糊项和按实际结算口子。',
        '涉及合同、付款和最终签约，仍要回到人工判断和书面确认。',
      ],
      actions: [
        { label: '先做报价初筛', href: '/tools/quote-check', kind: 'tool' },
        { label: '查风险词典', href: '/risk-dictionary', kind: 'resource' },
  ...
  },
  {
    ...
    payload: {
      ...
      actions: [
        ...
        { label: '先看服务价格', href: '/services/renovation', kind: 'service' },
...
function fallbackAnswer(message: string, locale: 'zh' | 'en'): ChatReplyPayload {
  ...
  return locale === 'en'
    ...
    : {
        ...
        actions: [
        { label: '看检查模板', href: '/checklists', kind: 'resource' },
        { label: '打开报价初筛', href: '/tools/quote-check', kind: 'tool' },
          { label: '直接联系 Zeno', href: '/contact', kind: 'contact' },
        ],
