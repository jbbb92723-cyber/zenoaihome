...
- 服务价格（报价风险初筛 / 合同风险初筛 / 报价对比判断）：/services/renovation
- 联系 Zeno：/contact
- 文章博客：/blog
...
const ROUTE_LABELS: Record<'zh' | 'en', Record<string, string>> = {
  zh: {
    ...
    '/services/renovation#quote-entry': '了解报价风险初筛',
    '/services/renovation#quote-standard': '了解合同风险初筛',
    '/services/renovation#quote-deep': '了解报价对比判断',
...
const FOLLOW_UPS_ZH: Record<IntentKey, string[]> = {
  ...
  service: ['报价初筛适合什么情况', '合同初筛和报价对比区别是什么', '我先用工具还是先咨询'],
...
const KNOWLEDGE_ZH: Array<{ pattern: RegExp; intent: IntentKey; payload: ChatReplyPayload }> = [
  ...
  },
  {
    ...
    intent: 'service',
    payload: {
      reply: '现在主线只保留三项人工判断：报价风险初筛、合同风险初筛、报价对比判断。不是所有情况都需要直接买服务，能先用免费初筛和检查模板看清的，先看清再说。',
      bullets: [
        '已经拿到装修报价但看不懂，优先看报价风险初筛。',
        '准备签合同但担心承诺没写清，优先看合同风险初筛。',
        '手里有 2-3 家报价不知道怎么选，优先看报价对比判断。',
      ],
      actions: [
