...
const ROUTE_LABELS: Record<'zh' | 'en', Record<string, string>> = {
  zh: {
    ...
    '/services/renovation#baojia-shenhe': '了解报价单审核',
    '/tools/budget-risk': '做预算风险自测',
    '/tools/budget-structure': '先看预算结构',
...
const FOLLOW_UPS_ZH: Record<IntentKey, string[]> = {
  ...
  quote: ['你先教我怎么看漏项', '这种按实际发生怎么算风险', '需要人工帮我看报价单吗'],
  service: ['报价单审核适合什么情况', '预算咨询会帮我看到哪一步', '我先用工具还是先咨询'],
...
const KNOWLEDGE_ZH: Array<{ pattern: RegExp; intent: IntentKey; payload: ChatReplyPayload }> = [
  {
    ...
    payload: {
      ...
      actions: [
        { label: '先做预算风险自测', href: '/tools/budget-risk', kind: 'tool' },
  ...
  },
  {
    ...
    intent: 'quote',
    payload: {
      reply: '报价单先别盯总价，先看它有没有故意留口子。看不清这些口子，后面再努力控预算也会很被动。',
      bullets: [
  ...
  },
  {
    ...
    intent: 'service',
    payload: {
      reply: '现在主要是三类支持：报价单审核、预算结构诊断、AI 工作流咨询。不是所有情况都需要直接买服务，能先用工具和清单看清的，先看清再说。',
      bullets: [
        '报价单已经在手上，优先看报价单审核。',
