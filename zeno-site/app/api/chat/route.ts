...
- 服务价格（¥99 报价风险初查 / ¥299 标准报价快审 / ¥699 签约前深度判断）：/services/renovation
- 联系 Zeno：/contact
- 文章博客：/blog
...
const ROUTE_LABELS: Record<'zh' | 'en', Record<string, string>> = {
  zh: {
    ...
    '/services/renovation#quote-deep': '了解签约前深度判断',
...
const KNOWLEDGE_ZH: Array<{ pattern: RegExp; intent: IntentKey; payload: ChatReplyPayload }> = [
  ...
  },
  {
    ...
    intent: 'service',
    payload: {
      reply: '现在主线只保留三项人工判断：¥99 报价风险初查、¥299 标准报价快审、¥699 签约前深度判断。不是所有情况都需要直接买服务，能先用免费初筛和检查模板看清的，先看清再说。',
      bullets: [
        '只有几行重点报价，优先看 ¥99 报价风险初查。',
        '已有完整报价、准备继续谈或签约，优先看 ¥299 标准报价快审。',
        '报价、合同和付款节点都已经有，且临近签约，再看 ¥699 签约前深度判断。',
      ],
      actions: [
