...
- 报价风险自查指南：/pricing/baojia-guide
- 预算分配工具：/tools/budget-structure
- 超预算原因自测：/tools/budget-risk
- 资料与清单：/resources
- 服务（报价快审 / 签约前决策包 / 预算取舍诊断）：/services/renovation
- 联系 Zeno：/contact
- 文章博客：/blog
...
const ROUTE_LABELS: Record<'zh' | 'en', Record<string, string>> = {
  zh: {
    ...
    '/contact': '直接联系 Zeno',
    '/pricing/baojia-guide': '看报价风险自查指南',
    '/resources': '去资料库拿清单',
...
const KNOWLEDGE_ZH: Array<{ pattern: RegExp; intent: IntentKey; payload: ChatReplyPayload }> = [
  ...
  },
  {
    ...
    payload: {
      ...
      actions: [
        { label: '先做报价初筛', href: '/tools/quote-check', kind: 'tool' },
        { label: '看报价风险自查指南', href: '/pricing/baojia-guide', kind: 'article' },
  ...
  },
  {
    ...
    intent: 'service',
    payload: {
      reply: '现在主要是三类人工支持：报价风险快审、签约前决策包、预算取舍诊断。不是所有情况都需要直接买服务，能先用工具、清单和 ¥39 指南看清的，先看清再说。',
      bullets: [
        '只是不确定报价重点风险，优先看报价风险快审。',
        '报价、预算、合同和付款节点一起乱，优先看签约前决策包。',
        '卡在预算分配和超支风险，优先看预算取舍诊断。',
      ],
      actions: [
  ...
  },
  {
    ...
    payload: {
      ...
      actions: [
        { label: '先做报价初筛', href: '/tools/quote-check', kind: 'tool' },
        { label: '拿报价审核清单', href: '/resources#baojia-shenhe-qingdan', kind: 'resource' },
        { label: '看报价风险自查指南', href: '/pricing/baojia-guide', kind: 'resource' },
      ],
