# 内容结构化系统

这是一个把原始内容资产加工为「内容单元」的新工程。

## 先看哪里

1. `SOURCE_OF_TRUTH.md`
2. `AGENTS.md`
3. `03-处理状态/处理状态总览.md`

## 当前默认目标

- 新工程目录已建立
- 规则文件、模板文件、状态文件已建立
- 原始素材副本已复制到 `01-原始素材区/完整副本/`
- 已建立来源注册、原始索引、待处理清单
- 已完成首批样本文稿的内容单元抽取
- 已建立关系索引与关系总览
- 已建立去重候选索引与去重总览
- 已建立主题地图和选题装配层
- 已建立系统总览脚本
- 已建立角色经营映射规则、角色索引、角色总图和首轮 30 天实践入口
- 已建立“框架到 SOP”转化规则、执行画布和装修报价判断最小交付示例
- 已建立框架到 SOP 执行台账，并将 OPC 内容获客策略落成专业判断公开化内容最小闭环

## 当前真实基线

- 内容单元：以 `node 07-脚本与工具/summarize-system.js` 输出为准
- 主题地图：以 `node 07-脚本与工具/summarize-system.js` 输出为准
- 装配稿：以 `node 07-脚本与工具/summarize-system.js` 输出为准
- 关系总数：以 `03-处理状态/关系总览.md` 为准
- 去重候选：以 `03-处理状态/去重与冲突总览.md` 为准

说明：动态统计以脚本和状态文件为准；下面只保留带日期的运行快照，不作为长期固定数字。

## 当前运行边界（2026-08-14）

- 当前基线：136 条原始素材索引、131 条待处理、5 条已处理；内容单元 50 个、主题地图 4 张、装配稿 59 份。
- 当前仍是样本 / 分批模式，尚未全量结构化。
- 角色经营层只负责调用、实践、证据和回流，不改变 `QST / CON / OPI / CAS / SOL` 内容单元规则。
- 问题单元当前存放在 `02-内容单元库/QUE-问题单元/`；`QST` 是内容单元 ID 前缀，不是要求新建的目录名。
- `canonical: true` 不等于已验证、可教学或可产品化。

## 当前核心目录

- `00-规则与索引/`
- `01-原始素材区/`
- `02-内容单元库/`
- `03-处理状态/`
- `04-模板/`
- `05-主题地图/`
- `06-选题装配/`
- `07-脚本与工具/`

框架落地入口：

- 规则：`00-规则与索引/框架到SOP转化规则.md`
- 画布：`07-模板库/框架到SOP画布.md`
- 示例：`04-产品服务/SOP-20260816_装修报价判断最小交付.md`
- 执行台账：`03-处理状态/框架到SOP执行台账.md`
- 内容获客示例：`04-产品服务/SOP-20260816_专业判断公开化内容最小闭环.md`
- 判断回写卡：`07-模板库/真实判断回写卡.md`
- 首条 SOP 运行记录：`10_运行记录与复盘/2026-08-16_2026-BJ-002_报价判断SOP试运行记录.md`

## 处理入口

- 批量生成来源候选：`node 07-脚本与工具/generate-source-registry.js`
- 重建原始索引与待处理清单：`node 07-脚本与工具/rebuild-processing-ledger.js`
- 自动抽取首批样本：`node 07-脚本与工具/extract-sample-units.js --help`
- 基于真实单元重组新选题：`node 07-脚本与工具/assemble-topic-from-units.js --title '你的选题' ...`
- 按选题自动推荐第一版装配：`node 07-脚本与工具/assemble-topic-from-units.js --title '年轻人怎么赚钱' --auto --top 3`
- 生成关系索引：`node 07-脚本与工具/generate-link-map.js`
- 生成去重候选：`node 07-脚本与工具/generate-duplicate-candidates.js`
- 补全 `Obsidian` 链接：`node 07-脚本与工具/fill-obsidian-links.js`
- 输出系统总览：`node 07-脚本与工具/summarize-system.js`
- 严格审计内容单元：`node 07-脚本与工具/audit-content-units.js --check`

## 角色经营入口

- 映射规则：`00-规则与索引/角色经营映射规则.md`
- 角色索引：`03-处理状态/角色经营索引.csv`
- 角色总图：`05-主题地图/赞诺角色经营总图.md`
- 首轮实践：`04-产品服务/赞诺角色经营体系首轮30天实践.md`

角色查询顺序是：先查角色索引，再读内容单元和方法卡，随后到项目 / 服务 / 发布 / 星火者记录找证据；不要只根据索引摘要宣布方法有效。

## Obsidian 链接约定

- `frontmatter` 中继续保留单元 `id`，供脚本、索引和关系字段使用
- 正文里引用其他内容单元、主题地图或装配稿时，统一使用 `Obsidian` `[[文件名]]` 直链
- 如果新增或修改了大量正文引用，可运行：`node 07-脚本与工具/fill-obsidian-links.js`

## 当前可验证能力

- 可以按语义块抽出多个真实单元，而不是每篇固定生成占位文件
- 可以生成可点击的主题地图与装配稿，并直接在 `Obsidian` 中跳转
- 可以基于现有内容单元，跨主题重组出新的选题装配稿
- 自动推荐不再只会回到原主题内部，已经可以给宽题拉出跨主题主轴

## 当前治理状态

- 2026-08-14 严格审计：50 个内容单元错误 0，警告 3。
- 3 个警告来自旧样本的待人工补全字段，不能据此宣布对应方法已验证。
- `SRC-EXT-MDL-001` 至 `008` 已登记到唯一来源注册表；OCR 文本只是当前定位副本，原件、页码和版权边界仍待复核。
